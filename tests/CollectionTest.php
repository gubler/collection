...
use Gubler\Collection\Contracts\Jsonable;
use Gubler\Collection\Contracts\Arrayable;
...
class CollectionTest extends TestCase
{
    ...
    public function testExcept()
    {
        ...
        $this->assertEquals(['first' => 'Taylor'], $data->except(Collection::from(['last', 'email', 'missing']))->all());
    ...
    {
        $collection = Collection::from(null);
    ...
    {
        $collection = Collection::from(['foo' => 'bar']);
    ...
    {
        $collection = Collection::wrap(Collection::from(['foo']));
        $this->assertEquals(['foo'], $collection->all());
    }

    public function testWrapWithCollectionSubclass()
    {
        $collection = TestCollectionSubclass::wrap(Collection::from(['foo']));
        $this->assertEquals(['foo'], $collection->all());
    ...
    public function testOnly()
    {
        ...
        $this->assertEquals(['first' => 'Taylor'], $data->only('first', 'missing')->all());
        $this->assertEquals(['first' => 'Taylor'], $data->only(Collection::from(['first', 'missing']))->all());

        $this->assertEquals(['first' => 'Taylor', 'email' => 'taylorotwell@gmail.com'], $data->only(['first', 'email'])->all());
        $this->assertEquals(['first' => 'Taylor', 'email' => 'taylorotwell@gmail.com'], $data->only('first', 'email')->all());
        $this->assertEquals(['first' => 'Taylor', 'email' => 'taylorotwell@gmail.com'], $data->only(Collection::from(['first', 'email']))->all());
    }
    ...
    public function testHigherOrderCollectionMap()
    {
        ...
        $collection = Collection::from([$person1, $person2]);

        $this->assertEquals(['Taylor', 'Yaz'], $collection->map->name->toArray());

        $collection = Collection::from([new TestSupportCollectionHigherOrderItem, new TestSupportCollectionHigherOrderItem]);
